...
//  long startMillis_sys = millis();
  ...
  while (1)
  {
     ...
     */
      ...
      if (!clap_cnt_confirming) //if not clap confirming count, it is clap command count
      {
  ...
  //      Serial.print("  clap_cnt >0 ");Serial.println(millis()-startMillis_TB);
      ...
      if (clap_cnt_confirming) // To confirm the command count with in clap confirming 
      {
